...
impl<'b> DaoTransaction<'b> {
    pub async fn get_account_by_account_key(&self, 
                                            account_key: &String) -> Result<Account, DaoError> {
    ...
    pub async fn get_account(&self, 
                             account_id: i32) -> Result<Option<Account>, DaoError> {
    ...
    pub async fn get_accounts(&self, 
                              account_ids: Vec<i32>) -> Result<HashMap<i32, Account>, DaoError> {
        let mut query_string: String = "".to_owned();
        query_string.push_str(ACCOUNT_QUERY);
